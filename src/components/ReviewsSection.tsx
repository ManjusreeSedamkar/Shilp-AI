    ...
    <div className="bg-white rounded-2xl p-4 shadow-sm border border-stone-200 space-y-3">
      ...
      <div className="flex flex-wrap sm:flex-nowrap items-center justify-between gap-2">
        ...
        <button
          ...
          className="px-3 py-1.5 rounded-xl bg-saffron-600 hover:bg-saffron-700 text-white text-xs font-bold flex items-center gap-1.5 transition-colors"
        >
      ...
      {/* Review Form */}
      {showForm && (
        <div className="p-3.5 bg-amber-50/40 border border-amber-200 rounded-2xl space-y-2.5">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  <Star className={`w-5 h-5 ${star <= rating ? 'fill-amber-400 text-amber-400' : 'fill-stone-300 text-stone-300'}`} />
                </button>
              ))}
            </div>
          </div>
          ...
          <textarea
            ...
            className="w-full px-3 py-2 bg-white border border-stone-200 rounded-xl text-xs text-stone-800 focus:ring-2 focus:ring-saffron-500 resize-none"
          />
          ...
          <div className="flex justify-end">
            <button
              ...
              className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 disabled:opacity-40 text-white text-xs font-bold flex items-center gap-1.5"
            >
      ...
      {reviews.length === 0 ? (
        ...
        <div className="space-y-2.5">
          {reviews.map((review) => (
            <div key={review.id} className="p-3 bg-stone-50 rounded-xl border border-stone-200 space-y-1.5">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                ...
                <div className="text-right">
                  ...
                  <span className="text-[9px] text-stone-400 block">{review.createdAt}</span>
                </div>
              </div>
