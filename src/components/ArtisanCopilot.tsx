...
import { Send, Mic, MicOff, Volume2, Sparkles, Bot, User, ArrowRight, CheckCircle2, ShoppingCart } from 'lucide-react';
...
export const ArtisanCopilot: React.FC<ArtisanCopilotProps> = ({
  ...
  const [messages, setMessages] = useState<CopilotMessage[]>([
    ArtisanCopilotService.getInitialGreeting(isHindi ? 'hi' : 'en')
  ]);
  ...
  useEffect(() => {
    setMessages([ArtisanCopilotService.getInitialGreeting(isHindi ? 'hi' : 'en')]);
  }, [language]);
  ...
  const handleSendMessage = (textToSend?: string) => {
    ...
    // Simulate Copilot processing and voice reply
    setTimeout(() => {
      const { reply } = ArtisanCopilotService.processArtisanInput(text, newHistory, isHindi ? 'hi' : 'en');
      setMessages((prev) => [...prev, reply]);

      // Auto-speak copilot reply for low literacy
      if (reply.audioText) {
        VoiceCatalogerEngine.speak(reply.audioText, isHindi ? 'hi-IN' : 'en-IN');
      }
    }, 600);
  };
  ...
  const toggleVoiceInput = () => {
    if (isRecording) {
      ...
      setIsRecording(true);
      // Simulated voice recognition or prompt
      setTimeout(() => {
        setIsRecording(false);
  ...
  const handleSpeak = async (msg: CopilotMessage) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-stone-200 flex flex-col h-[700px] overflow-hidden">
          ...
          <div>
            <div className="flex items-center gap-1.5">
              <h3 className="font-bold text-sm text-white">
                {isHindi ? 'शिल्प-AI बिजनेस मैनेजर' : 'SHILP-AI Virtual Business Manager'}
              </h3>
            ...
            <p className="text-[11px] text-stone-300">
              {isHindi ? 'कम डिजिटल साक्षरता के लिए विशेष वॉयस असिस्टेंट' : 'Conversational AI for Low-Literacy Artisans'}
            </p>
          </div>
        </div>

        <div className="flex items-center space-x-1">
          <span className="text-[11px] text-emerald-300 bg-emerald-950/80 px-2 py-0.5 rounded-full border border-emerald-500/30 flex items-center gap-1">
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-stone-50/50">
        {messages.map((msg) => {
          ...
          return (
            ...
            >
              <div
                className={`max-w-[88%] sm:max-w-[80%] rounded-2xl p-3.5 shadow-sm text-xs ${
                  isUser
                    ? 'bg-gradient-to-r from-saffron-600 to-amber-600 text-white rounded-br-none'
                    : 'bg-white text-stone-800 border border-stone-200 rounded-bl-none'
                }`}
              >
                {/* Sender Tag & Audio Readout Icon */}
                <div className="flex items-center justify-between mb-1 text-[10px] opacity-75">
                  <span className="font-bold flex items-center gap-1">
                    ...
                    {isUser ? (isHindi ? 'आप (कारीगर)' : 'You (Artisan)') : 'SHILP Copilot'}
                  </span>
                  {!isUser && (
                    <button
                      onClick={() => handleSpeak(msg)}
                      className="hover:opacity-100 flex items-center gap-1 text-saffron-700 font-semibold"
                      title="Read aloud"
                    >
                      <Volume2 className={`w-3 h-3 ${isSpeakingMessageId === msg.id ? 'animate-bounce text-saffron-600' : ''}`} />
                      <span>{isSpeakingMessageId === msg.id ? 'Speaking...' : 'Listen'}</span>
                    </button>
                  )}
                </div>
                ...
                {msg.actionCard?.type === 'listing_ready' && msg.actionCard.data && (
                  <div className="mt-3 p-3 bg-stone-50 rounded-xl border border-stone-200 text-stone-900 space-y-2">
                    <div className="flex items-center gap-2">
                      <img
                        src={msg.actionCard.data.enhancedImage}
                        alt="Product"
                    ...
                    <div className="pt-2 border-t border-stone-200 flex justify-between items-center">
                      <span className="text-[10px] text-stone-500 flex items-center gap-1">
                        ...
                        {isHindi ? 'जीआई व MoSJE प्रमाणित' : 'GI & MoSJE Ready'}
                      </span>
                      <button
      ...
      {/* Quick Artisan Speech Suggestion Chips */}
      <div className="px-3 py-2 bg-stone-100/70 border-t border-stone-200 flex items-center space-x-2 overflow-x-auto no-scrollbar text-xs">
        ...
        >
          <span>💡</span>
          <span>{isHindi ? 'थोक मोलभाव सलाह' : 'Bulk negotiation advice'}</span>
        </button>
        <button
        ...
        >
          <span>💳</span>
          <span>{isHindi ? 'जीएसटी नियम' : 'GST exemption rules'}</span>
        </button>
        <button
          onClick={() => handleSendMessage(isHindi ? 'मिट्टी के बर्तनों और नाजुक शिल्प की सुरक्षित पैकिंग कैसे करें?' : 'How should I safely pack fragile pottery for courier?')}
          className="whitespace-nowrap px-2.5 py-1 rounded-full bg-white hover:bg-purple-50 text-stone-700 hover:text-purple-800 text-[11px] font-medium border border-stone-200 shadow-2xs transition-colors flex items-center gap-1"
        >
          <span>📦</span>
          <span>{isHindi ? 'सुरक्षित पैकिंग' : 'Safe courier packing'}</span>
        </button>
        <button
          onClick={() => handleSendMessage(isHindi ? CRAFT_PRESETS[0].sampleVoiceHindi : CRAFT_PRESETS[0].sampleVoiceEnglish)}
          className="whitespace-nowrap px-2.5 py-1 rounded-full bg-white hover:bg-saffron-50 text-stone-700 hover:text-saffron-800 text-[11px] font-medium border border-stone-200 shadow-2xs transition-colors flex items-center gap-1"
        >
          <span>🥻</span>
          <span>{isHindi ? 'पोचमपल्ली साड़ी बेचना है' : 'Sell Pochampally Saree'}</span>
        </button>
      </div>
      ...
      <div className="p-3 bg-white border-t border-stone-200">
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            title={isHindi ? 'बोलकर बताएं' : 'Speak to Copilot'}
          >
          ...
          <input
            ...
            placeholder={
              isHindi
                ? 'माइक दबाएं या लिखें: "यह रेशम की साड़ी है, 5 दिन लगे, ₹2,500 लागत..."'
                : 'Tap mic or type: "This is a handwoven saree, took 5 days, silk cost ₹2,500..."'
            }
            className="flex-1 px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-xl text-xs text-stone-800 focus:outline-none focus:ring-2 focus:ring-saffron-500"
          />
