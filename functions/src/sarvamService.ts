...
export interface SarvamTranslateRequestPayload {
  input: string;
...
export interface SarvamTranslateApiResponse {
  request_id?: string;
  translated_text: string;
  source_language_code?: string;
}

export interface SarvamErrorResponse {
  message?: string;
  code?: string | number;
  request_id?: string;
  detail?: string;
...
export async function callSarvamTranslate(
  ...
  targetCode: string,
  apiKey: string
): Promise<SarvamTranslateApiResponse> {
  if (!apiKey || apiKey.trim().length === 0) {
    throw new Error("SARVAM_API_KEY is missing or empty.");
  }

  const payload: SarvamTranslateRequestPayload = {
    input: text,
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (isJson) {
        try {
          const errorBody = (await response.json()) as SarvamErrorResponse;
    ...
    const data = (await response.json()) as SarvamTranslateApiResponse;
